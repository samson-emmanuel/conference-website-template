...
// setupCountdown("morning-session-timer1", new Date("2025-01-29T14:00:00").getTime());
...
// setupCountdown("morning-morning-session-timer1", new Date("2025-01-31T09:00:00").getTime());
...
const setupCountdown = (id, eventTime) => {
    ...
    const updateTimer = () => {
        ...
        if (timeLeft <= 0 && !isHappeningNow) {
            timerElement.innerHTML = "Happening Now!";
            timerElement.style.color = "green";
            cardElement.classList.add("happening-now");
            isHappeningNow = true;
            ...
            setTimeout(() => {
                if (isHappeningNow) {
                    ...
                    cardElement.classList.remove("happening-now");
